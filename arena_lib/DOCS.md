...
* [1. Minigame configuration](#1-minigame-configuration)
	...
	* [1.11 Endless minigames](#111-endless-minigames)
	* [1.12 Custom entrances](#112-custom-entrances)
	* [1.13 Extendable editor](#113-extendable-editor)
	* [1.14 Things you don't want to do with a light heart](#114-things-you-dont-want-to-do-with-a-light-heart)
* [2. Arenas](#2-arenas)
	* [2.1 Storing arenas](#21-storing-arenas)
	* [2.2 Setting up an arena](#22-setting-up-an-arena)
		* [2.2.1 Editor](#221-editor)
		* [2.2.2 Via Code](#222-via-code)
			...
			* [2.2.2.12 Region](#22212-region)
				* [2.2.2.12.1 Schematics](#222121-schematics)
	...
	* [2.6 Fake players](#26-fake-players)
...
* `description`: (string) optional description for your minigame. Default is `""`
...
* `min_version`: (int) the minimum Luanti version required to play the minigame, expressed in Luanti `protocol_version` (e.g. 5.8 is 43). By default is `-1`. For a better implementation, [this](https://github.com/luanti-org/luanti/issues/14151) is needed
...
* `chat_settings`: (table) several chat options
  * Default fields are:
    ```lua
    {
      ...
      isolate_chat = true -- whether the chat of the arena is isolated from the rest of the world
      can_interact_with_spectators = false -- whether spectators and players' chat should be shared
    }
    ```
...
* `player_aspect`: (table) changes the aspect of every player entering a game. It supports a few parameters from Luanti [Object Properties](https://api.luanti.org/definition-tables/), namely `visual`, `visual_size`, `mesh`, `textures`, `collisionbox` and `selectionbox`
...
* `hp_max`: (int) the maximum HP of players (life will be refilled either way)
* `fov`: (int) changes the fov of every player
* `camera_mode`: (string) changes the camera mode as in Luanti's `player:set_camera()`. Default is `nil`
  * Due to a Luanti limitation, even if arena_lib will set the pre-arena camera mode once leaving, it cannot know which mode the player was currently in. Considering that the first person is the most used one, in case of pre-arena camera mode = `"any"`, it'll force the first person first; and, after a step, `set_camera({mode = "any"})`. See [this issue](https://github.com/luanti-org/luanti/issues/16211)
...
* `endless`: (bool) whether the minigame is of type endless. If `true`, `join_while_in_progress` is automatically `true` and `min_players` is `0`. Default is `false`. For further information, check out the specific section [1.11 Endless minigames](#111-endless-minigames)
...
* `can_build`: (bool) whether players are allowed to place or dig nodes during a match. Default is `false`
  * If `true`, it'll bypass protection mods - so beware when placing arenas
...
* `keep_attachments`: (bool) whether to keep the entities attached to the player. Default is `false`
  * If `false`, entities are deleted when entering, and re-created (alongside their properties) when leaving. Also, in-game attachments are deleted when leaving
  * No matter the value, players are always detached from any parent of theirs both when entering and leaving
* `keep_physics`: (bool) whether to keep the physics of the players. If `false`, it'll use physics as declared in [1.10 Physics](#110-physics). Default is `false`
...
* `in_game_physics`: (table) per field override of player physics values, following the Luanti `physics_override` structure. See [1.10 Physics](#110-physics) to learn more.
  * If `keep_physics = true`, `in_game_physics` won't work
* `damage_modifiers`: (table) an override of players' armor groups, as declared in the [Luanti API](https://api.luanti.org/groups/#objectref-armor-groups)
* `disabled_damage_types`: (table) contains which damage types will be disabled once in a game. Damage types are strings, the same as in `reason.type` in the [Luanti API](https://api.luanti.org/core-namespace-reference/#global-callback-registration-functions) (e.g. `{"fall", "punch"}`)
* `hud_flags`: (table) contains which HUD flags will be enabled and which won't once in game. It follows the same syntax of `hud_set_flags(..)` in the [Luanti API](https://api.luanti.org/class-reference/#player-only-no-op-for-other-objects) (e.g. `{hotbar = true, minimap = false}`). If a flag is not declared, the current player's flag will be kept
...
* `arena_lib.on_end(mod, function(arena, winners, is_forced))`: same as above. It's called after flushing the arena (both from players and data)
  ...
  * If you want to teleport players onto a custom position, don't do it here. Instead, change the `custom_return_point` of the arena. However, if you really need to teleport players onto different positions, due to current LT limitations (see [here](https://github.com/luanti-org/luanti/issues/12092)), you have to do that after at least 0.2 seconds
...
* `arena_lib.on_join(mod, function(p_name, arena, as_spectator, was_spectator))`: called when a user joins an ongoing match.
  * Due to current LT limitations, calling `set_pos()` on a player is not possible, unless it's done after at least 0.2 seconds (see [here](https://github.com/luanti-org/luanti/issues/12092))
  ...
  * `was_spectator` returns `true` if the user was spectating `arena` when joining as an actual player. Such players never really leave the arena. Useful e.g. to avoid recreating all the HUDs of a minigame
...
* `arena_lib.on_region_trespass(mod, function(arena, players_outside))`: called every second if a region is declared. `players_outside` is a list of objects outside the region.
  * If not declared, by default players will be killed when trespassing
* `arena_lib.on_eliminate(mod, function(arena, p_name, xc_name, p_properties, p_in_game_data))`: called when a player is eliminated (see `arena_lib.remove_player_from_arena(..)`)
  ...
  * `p_in_game_data` is a table of additional in-game data regarding `p_name`. Fields:
    * `last_pos`: (table) last position before ending up in spectate mode/leaving the match
* `arena_lib.on_quit(mod, function(arena, p_name, is_spectator, reason, p_properties, p_in_game_data))`: called when a player/spectator quits (or is forced to quit) from an ongoing match
  ...
  * `p_in_game_data`: see `on_eliminate`
...
* `arena_lib.register_on_eliminate(function(mod, arena, p_name, xc_name, p_properties, p_in_game_data))`
* `arena_lib.register_on_quit(function(mod, arena, p_name, is_spectator, reason, p_properties, p_in_game_data))`
...
arena_lib.register_on_prejoin_queue(function(mod, arena, p_name)
  ...
  if myservermod.is_event_active() then
	core.chat_send_player(p_name, "There is a special event in progress, minigames will be back once it ends!")
	return
  end
...
On the contrary, if you need to play positional sounds, just use `audio_lib.play_sound(..)` or (advised against) `core.sound_play(..)`
...
Keep also in mind that nametags are stored when entering and restored when leaving. This means that nametags can be used as a custom HUD too (e.g. to display the score of a certain player above their head), using the default Luanti API.
...
* `arena_lib.join_arena(mod, p_name, arenaID, as_spectator)`: adds `p_name` to the ongoing match in `arena`, if any. Returns `true` if successful
...
* `arena_lib.get_arena_spawners_count(arena, <teamID>)`: returns the total amount of spawners declared in the specified arena. If `teamID` is specified, it'll only return the amount of spawners belonging to that team
* `arena_lib.get_random_spawner(arena, <teamID>, <pos_and_rot>)`: returns a random spawner declared in the specified arena
  ...
  * If `teamID` is specified, it only considers the ones belonging to that team
...
* `arena_lib.get_players_in_team(arena, teamID, <to_player>)`: returns a table containing as value either the names of the players inside the specified team or, if `to_player` is `true`, the players themselves
* `arena_lib.get_active_teams(arena)`: returns an ordered table having as values the ID of teams that are not empty
* `arena_lib.get_teamID(p_name)`: returns the ID of the team `p_name` is in, if any
...
* `arena_lib.get_party_members_spectating(p_name, <arena>)`: returns a table containing as value the names of all the party members of `p_name` currently spectating in a minigame (or in `arena` only, if specified)ab

### 1.10 Physics
By default, arena_lib handles player physics both when entering and when leaving an arena. If you don't want that, skip to the last paragraph.  

When entering, default physics assigned to players is the same listed in Luanti docs (speed 1, speed_walk 1 etc.). However, modders can change one or more values from the minigame declaration, by specifying the `in_game_physics` ingredient. Keep in mind that if a value is not declared, the default one will be used instead; e.g. declaring `in_game_physics = {speed = 1.5}` would set players physics to `{speed = 1.5, speed_walk = 1, speed_climb = 1, etc}`.  

When leaving, arena_lib uses the `arena_lib.SERVER_PHYSICS` setting contained in the arena_lib world folder to set players physics once outside. If you think people might have had physics modifiers before entering (e.g. having a 10 minute speed buff), be sure to use the `arena_lib.register_on_quit` and `register_on_end` global callback to run the checks needed and potentially update their physics.  

Modders also have the option to use the in-world physics, so that arena_lib won't alter it neither when entering nor when leaving. To do that, just use the `keep_physics = true` ingredient in the minigame declaration.

### 1.11 Endless minigames
...
### 1.14 Things you don't want to do with a light heart
* Changing the technical name of your minigame when one or more arenas are declared. `arena_lib` won't be able to automatically convert your arenas database, so that must be done manually (it'll automatically convert the arenas `mod` parameter though)
...
* `mod` (string) the name of the minigame the arena is associated to, for easy cross-referencing. Automatically set when creating an arena
...
First of all, run `/arenas create <minigame> <arena>` to create an arena.  
To edit it, arena_lib comes with a built-in editor that makes admins' life easier. On the contrary, if you need to tweak some properties via e.g. an external mod or commands, you can use the functions listed in the [2.2.2 Via Code](#222-via-code) section.

### 2.2.1 Editor
arena_lib comes with a fancy editor, so you don't have to configure and memorise a lot of commands.  
...
#### 2.2.2 Via Code
If you don't want to rely on the editor, or you want both the editor and the ability to tweak arenas outside it, here are the functions lying beneath. Note that:
...
`arena_lib.change_teams_amount(sender, mod, arena_name, amount)` changes the amount of teams in a specific arena. It'll use the first N teams declared, where N is `amount`. It returns `true` if successful.
...
To set an entrance, use `arena_lib.set_entrance(sender, mod, arena_name, action, ...)`. For further documentation, see [1.12 Custom entrances](#112-custom-entrances).  
...
##### 2.2.2.11 Lighting
NOTE: EXPERIMENTAL FEATURE. EXPECT BREAKAGE IN THE FUTURE (according to the direction Luanti will choose to go with lighting)  
...
The soft reset system includes **liquid drain support**: when liquids (water, lava, etc.) are placed during a match, the system automatically detects and removes the entire liquid flow, including source blocks and flowing blocks that may have spread to adjacent areas. 

**Important**: the official arena map should not contain any liquids, as the system cannot distinguish between valid liquids (part of the original map) and invalid ones (placed by players during the match). All liquids will be removed during reset.

Due to Luanti limitations, not every node can be restored. For this reason, **do not** use the following approaches to alter the map or arena_lib won't be able to track the change:
* built-in ABMs / LBMs
* VoxelManip
* inventory nodes
...
2. schematics in Luanti are cached when loaded. Meaning that, if the schematic changes, the server needs to be rebooted in order to paste the new version
...
* `loading phase`: the pre-match. By default players get teleported in the arena and their life refilled, waiting for the game to start. Relevant callback: `on_load`
* `playing phase`: the actual game. Relevant callbacks: `on_start`, `on_join`
* `celebration phase`: the after-match. By default people stroll around for the arena knowing who won, waiting to be teleported. Life is refilled after teleportation. Relevant function: `arena_lib.load_celebration(..)`. Relevant callbacks: `on_celebration`, `on_end`.

### 2.3.1 Handling player properties (saving and restoring)
When a player joins a match, they're automatically detached from any entity they're attached to and their life fulled. This is also true when they leave a match. Additionally, when they leave their physics is set to the setting `arena_lib.SERVER_PHYSICS` (unless `hub` is installed) and their children deleted (unless `keep_attachments = true`).  
...
If a game is already in progress and a party member want to reach their mates who are already inside (e.g. the party member went offline during the loading phase), the entering party member will be added in the team of their group only if the amount of players in teams is the same.  
...
## 2.6 Fake players
> EXPERIMENTAL FEATURE: to properly support fake physical players this is needed: https://github.com/luanti-org/luanti/issues/13430. Expect breakage!

Fake *abstract* players are somehow¹ supported by arena_lib and you can use a mod like [fakelib](https://content.luanti.org/packages/OgelGames/fakelib/) to create them. arena_lib identifies fake players by checking whether there is a space inside their name, so be sure to put one if you don't want to see the server crash. This will likely change in the future, when the aforementioned issue is solved.

¹Some actions will probably crash the server, e.g. putting a fake player as a spectator
