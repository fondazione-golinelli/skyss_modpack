...
core.register_globalstep(function(dtime)
  for pl_name, pl_data in pairs(players_in_edit_mode) do
    local item = core.get_player_by_name(pl_name):get_wielded_item()
...
function arena_lib.register_editor_section(mod, def)
  ...
  core.register_tool(mod .. ":arenalib_editor_slot_custom", {
      description = name,
...
function arena_lib.enter_editor(sender, mod, arena_name)
  ...
  if arena.weather then
    ...
    players_in_edit_mode[sender].weather_ID = core.add_particlespawner(particles)
  end
  ...
  if spawners_amount > 0 or arena.pos1 then
    ...
    core.chat_send_player(sender, S("Wooosh!"))
  end
...
function arena_lib.quit_editor(player)
  ...
  if weather_ID then
    core.delete_particlespawner(weather_ID)
  end
...
function arena_lib.show_main_editor(player)
  ...
  if core.registered_items[mod .. ":arenalib_editor_slot_custom"] then
...
function arena_lib.set_editor_particlespawner(p_name, ID)
  ...
  if p_data.weather_ID then
    core.delete_particlespawner(p_data.weather_ID)
  end
