...
core.register_tool("arena_lib:customise_weather", {
    ...
    on_use = function(itemstack, user, pointed_thing)
      ...
      if arena.weather then
        ...
        temp_particles[p_name].ID = core.add_particlespawner(arena.weather)
      ...
      core.show_formspec(p_name, "arena_lib:weather", get_weather_formspec(p_name, curr_section[p_name]))
    end
})
...
function set_temp_particles(p_name)
  ...
  if temp_particles[p_name].ID then
    core.delete_particlespawner(temp_particles[p_name].ID)
  end
  ...
  particles.playername = p_name
  particles.attached = core.get_player_by_name(p_name)

  temp_particles[p_name].ID = core.add_particlespawner(particles)
end
...
core.register_on_player_receive_fields(function(player, formname, fields)
  ...
  -- se abbandona...
  if fields.quit then
    core.delete_particlespawner(temp_particles[p_name].ID or -1)
    ...
    if arena.weather then
      ...
      arena_lib.set_editor_particlespawner(p_name, core.add_particlespawner(particles))
    end
  ...
  temp_particles[p_name].amount = core.explode_scrollbar_event(fields.strength).value
