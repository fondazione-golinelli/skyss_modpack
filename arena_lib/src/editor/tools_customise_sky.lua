...
local function get_fog_params() end
...
local temp_util_values = {}             -- KEY: p_name, VALUE: {fog_distance, fog_start, fog_color}
...
core.register_tool("arena_lib:customise_sky", {
    ...
    on_use = function(itemstack, user, pointed_thing)
      ...
      core.show_formspec(p_name, "arena_lib:celestial_vault", get_celvault_formspec(p_name, "sky"))
    end
})
...
function get_celvault_formspec(p_name, section)
  local formspec = {
    ...
    "image_button[1.345,0;1,1;arenalib_sky_fog.png" .. (section == "fog" and "^[multiply:#777777" or "") .. ";fog_btn;]",
    "image_button[2.69,0;1,1;arenalib_sky_sun.png" .. (section == "sun" and "^[multiply:#777777" or "") .. ";sun_btn;]",
    "image_button[4.035,0;1,1;arenalib_sky_moon.png" .. (section == "moon" and "^[multiply:#777777" or "") .. ";moon_btn;]",
    "image_button[5.38,0;1,1;arenalib_sky_stars.png" .. (section == "stars" and "^[multiply:#777777" or "") .. ";stars_btn;]",
    "image_button[6.725,0;1,1;arenalib_sky_cloud.png" .. (section == "clouds" and "^[multiply:#777777" or "") .. ";clouds_btn;]",
    "container_end[]",
    "container[1,2.2]",
  ...
  elseif section == "fog" then
    fields = get_fog_params(p_name)
...
function get_fog_params(p_name)
  local temp_sky_fog = temp_sky_settings[p_name].sky.fog
  local is_fog_visible = temp_sky_fog.fog_distance ~= nil
  local fog = {"checkbox[0,0.78;enable_fog;" .. S("Fog") .. ";" .. tostring(is_fog_visible) .. "]"}

  if is_fog_visible then
    local fog_params = {
      "container[0,1.5]",
      "field[0,0.18;3.75,0.6;fog_distance;" .. S("Fog distance") .. ";" .. temp_sky_fog.fog_distance .. "]",
      "label[3.85,0;" .. S("Fog start") .. "]",
      "label[3.85,0.47;0]",
      "label[7.6,0.47;1]",
      "scrollbar[4.2,0.31;3.15,0.3;horizontal;fog_start;" .. temp_sky_fog.fog_start * 100 .. "]",
      "label[0,1;" .. S("Colour") .. "]",
      "dropdown[0,1.2;2.5,0.6;fog_color;" .. palette_str .. ";" .. (palette_id[colstr(temp_sky_fog.fog_color)] or 1) .. "]",
      "container_end[]",
    }
...
function get_stars_params(p_name)
  ...
  if are_stars_visible then
    local stars_params = {
      ...
      "label[0,2.15;" .. S("Seed (0 = random)") .. "]",
      "label[0,2.65;0]",
      "label[7.6,2.65;100]",
      "scrollbar[0.3,2.5;7.15,0.3;horizontal;stars_seed;" .. (temp_stars.star_seed or 0) .. "]", -- non c'è bisogno di metterne 4 milioni
      "container_end[]"
    }
...
function get_clouds_params(p_name)
  ...
  if not are_clouds_visible then
    ...
    clouds = {
      ...
      "label[0,1.1;" .. S("Bottom") .. "]",
      "dropdown[0,1.3;3.75,0.6;clouds_shadow;" .. palette_str .. ";" .. (palette_id[temp_clouds.shadow] or 1) .. "]",
      "container[0,2.8]",
      "label[0,-0.5;" .. S("Opacity") .. "]",
      "label[0,0;0]",
      "label[7.6,0;1]",
      "scrollbar[0.3,-0.15;7.15,0.3;horizontal;clouds_opacity;" .. alpha * 100 .. "]",
      "container_end[]",
      "container_end[]",
      -- density etc
      "container[0,3.4]",
...
core.register_on_player_receive_fields(function(player, formname, fields)
  ...
  elseif fields.reset then
    ...
    if fields.day_sky then
      core.show_formspec(p_name, "arena_lib:celestial_vault", get_celvault_formspec(p_name, "sky"))
    elseif fields.fog_distance then
      core.show_formspec(p_name, "arena_lib:celestial_vault", get_celvault_formspec(p_name, "fog"))
    elseif fields.sun_size then
      core.show_formspec(p_name, "arena_lib:celestial_vault", get_celvault_formspec(p_name, "sun"))
    elseif fields.moon_size then
      core.show_formspec(p_name, "arena_lib:celestial_vault", get_celvault_formspec(p_name, "moon"))
    elseif fields.stars_count then
      core.show_formspec(p_name, "arena_lib:celestial_vault", get_celvault_formspec(p_name, "stars"))
    else
      core.show_formspec(p_name, "arena_lib:celestial_vault", get_celvault_formspec(p_name, "clouds"))
    end
  ...
  if fields.fog_color then
    temp_sky_fog.fog_color = palette[fields.fog_color]
  end
  ...
  if fields.stars_seed then
    temp_stars.star_seed = core.explode_scrollbar_event(fields.stars_seed).value
  end

  -- nuvole
  -- Luanti è stupido perché alcuni campi li aggiorna sempre e altri no, quindi
  ...
  if fields.clouds_shadow then
    temp_clouds.shadow = palette[fields.clouds_shadow]
  end
  ...
  -- reale l'eventuale nebbia rimasta allə giocante nell'editor (LT richiede che
  -- la nebbia sia -1 per rimuoverla) e ne resetto il colore
  if not temp_sky_fog.fog_distance then
    temp_sky_fog.fog_distance = -1
    temp_sky_fog.fog_start = -1
    temp_sky_fog.fog_color = "#00000000" -- dalla documentazione: "default: "#00000000", which means no override"
    reset_fog = true
  end
