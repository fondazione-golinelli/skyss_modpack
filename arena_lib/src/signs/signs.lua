...
-- the basic `core.close_formspec` function, also calling it in `fields.quit` down below
local original_close = core.close_formspec
function core.close_formspec(p_name, formname)
...
local original_show = core.show_formspec
function core.show_formspec(p_name, formname, formspec)
...
arena_lib.register_entrance_type("arena_lib", "sign", {
  ...
  debug_output = function(entrance) return core.pos_to_string(entrance) end
})
...
signs_lib.register_sign("arena_lib:sign", {
  ...
  on_construct = function(pos)
    core.get_meta(pos):set_int("widefont", 1)
    end,

  -- cartello indistruttibile se c'è un'arena assegnata
  on_dig = function(pos, node, digger)
    if core.get_meta(pos):get_int("arenaID") ~= 0 then return end
  ...
  on_rightclick = function(pos, node, clicker, itemstack, pointed_thing)
    ...
    local mod = core.get_meta(pos):get_string("mod")
    local arenaID = core.get_meta(pos):get_int("arenaID")
    ...
    core.show_formspec(p_name, "arena_lib:infobox", get_infobox_formspec(mod, arenaID, clicker))
  end,
...
function add_sign(sender, mod, arena, pos)
  ...
  if core.get_node(pos).name ~= "arena_lib:sign" then
  ...
  core.get_meta(pos):set_string("mod", mod)
  core.get_meta(pos):set_int("arenaID", id)
...
function remove_sign(mod, arena)
  core.load_area(arena.entrance)

  local sign_meta = core.get_meta(arena.entrance)
  local id = arena_lib.get_arena_by_name(mod, arena.name)
  ...
  if core.get_node(arena.entrance).name == "arena_lib:sign" and sign_meta:get_string("mod") == mod and
                                                                    sign_meta:get_int("arenaID") == id then
    core.set_node(arena.entrance, {name = "air"})
  end
end
...
function update_sign(mod, arena)
  ...
  for pl_name, pl_data in pairs(displaying_infobox) do
    ...
    if pl_data.mod == mod and pl_data.arena_id == id then
      core.show_formspec(pl_name, "arena_lib:infobox", get_infobox_formspec(mod, id, core.get_player_by_name(pl_name)))
    end
  end
end
...
function get_infobox_formspec(mod, arenaID, player)
  ...
  -- sennò
    ...
    inside_col = "#d7ded7"
    pl_list = "hypertext[0.7,2.2;3.6,2;name;<global halign=center valign=middle><style size=16 font=mono color=#d7ded7>" .. S("No ongoing activity") .. "</style>]"
  end
  ...
  if core.check_player_privs(p_name, "arenalib_admin") then
    local admin_elems = {
      "tooltip[edit;" .. S("Edit") .. "]",
  ...
  local formspec = {
    ...
    "hypertext[1,1.59;3.45,1;author;<global valign=middle><style size=18 font=mono color=#5a5353>" .. F(arena.author) .. "</style>]",
    "hypertext[1,2.75;3.45,1;bgm;<global valign=middle><style size=18 font=mono color=#5a5353>" .. F(bgm_info) .. "</style>]",
...
core.register_on_player_receive_fields(function(player, formname, fields)
  ...
  -- a un problema di Luanti, tanto vale non dannarsi e aspettare che venga
  ...
  if fields.quit then
    core.close_formspec(p_name, formname) -- necessario per chiamare la funzione sovrascritta qui sopra, rimuovendo p_name da displaying_infobox
    return

  elseif fields.edit then
    if not core.get_player_privs(p_name).arenalib_admin then return end -- sanity check
    arena_lib.enter_editor(p_name, mod, arena.name)
    core.close_formspec(p_name, formname)
    return
