...
function arena_lib.enter_minigame_settings(sender, mod)
  ...
  core.show_formspec(sender, "arena_lib:minigame_settings", get_settings_formspec(sender, 1))
end
...
core.register_on_player_receive_fields(function(player, formname, fields)
  ...
  if fields.minigame_settings then
    ...
    local expl = core.explode_textlist_event(fields.minigame_settings)

    if expl.type == "DCL" or expl.type == "CHG" then
      core.show_formspec(p_name, "arena_lib:minigame_settings", get_settings_formspec(p_name, expl.index))
    end
  ...
  elseif fields.setting_overwrite or fields.key_enter then
    ...
    core.show_formspec(p_name, "arena_lib:minigame_settings", get_settings_formspec(p_name, sel_setting_attr[p_name].id))
