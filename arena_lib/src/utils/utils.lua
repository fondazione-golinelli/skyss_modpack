...
function arena_lib.print_info(p_name, msg)
  if not p_name or not core.get_player_by_name(p_name) then
...
function arena_lib.print_error(p_name, msg)
  if not p_name or not core.get_player_by_name(p_name) then
    -- se ci sono stringhe tradotte, le ripulisco
    -- TEMP?: se arena_lib dovesse richiedere LT 5.14+ in futuro, usa https://github.com/luanti-org/luanti/pull/16485 (idem per print info)
    if msg:find("\27%(T@[%w_-]+%)") then
      msg = msg:gsub("\27%(T@[%w_-]+%)", "")  -- removing \27(T@mod_name)
      msg = msg:gsub("\27F", "")
      msg = msg:gsub("\27E", "")
    ...
    core.log("error", "[ARENA_LIB] [!] " .. msg)
  else
    core.chat_send_player(p_name, core.colorize("#e6482e", S("[!] @1", msg)))
  end
end
...
function arena_lib.get_and_add_joining_players(arena, p_name)
  ...
  if arena.teams_enabled and (arena.in_game or (IS_PARTIES_ENABLED and parties.is_player_in_party(p_name))) then
    p_team_ID = assign_team(arena, p_name)
  end

  local players_to_add = {}

  -- potrei avere o unə giocante o un intero gruppo da aggiungere. Quindi per evitare
  -- mille if, metto a prescindere l3 giocanti in una tabella per iterare in
  ...
  if IS_PARTIES_ENABLED and parties.is_player_in_party(p_name) then
    for _, pl_name in ipairs(parties.get_party_members(p_name, true)) do
      if not arena_lib.is_player_playing(pl_name) then
        players_to_add[#players_to_add+1] = pl_name
      end
    end
  else
    players_to_add[#players_to_add+1] = p_name
  end
...
function assign_team(arena, p_name)
  ...
  -- se è in partita, si è in gruppo e c'è lo stesso numero di giocanti in ogni squadra,
  -- metti nella squadra dove il gruppo è presente
  if arena.in_game and IS_PARTIES_ENABLED and parties.is_player_in_party(p_name) and sorted_teams[1].amount == sorted_teams[#sorted_teams].amount then
    for _, pl_name in ipairs(parties.get_party_members(p_name, true)) do
