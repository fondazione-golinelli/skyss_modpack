...
function arena_lib.print_minigames(sender)
  ...
  str = str .. "-------------------------"
  core.chat_send_player(sender, str)
end
...
function arena_lib.print_arenas(sender, mod)
  ...
  for id, arena in pairs(arena_lib.mods[mod].arenas) do
    n = n+1
    core.chat_send_player(sender, S("ID: @1, name: @2", id, arena.name))
  end

  core.chat_send_player(sender, S("Total arenas: @1", n))
end
...
function arena_lib.print_arena_info(sender, mod, arena_name)
  ...
  if arena.teams_enabled then
    min_players_per_team = core.colorize("#eea160", S("Players required per team: @1", core.colorize("#cfc6b8", arena.min_players))) .. "\n"
    max_players_per_team = core.colorize("#eea160", S("Players supported per team: @1", core.colorize("#cfc6b8", arena.max_players))) .. "\n"
    ...
    players_inside_per_team = core.colorize("#eea160", S("Players inside per team: @1", core.colorize("#cfc6b8", players_inside_per_team))) .. "\n"
    if mod_ref.spectate_mode then
      spectators_inside_per_team = core.colorize("#eea160", S("Spectators inside per team: @1", core.colorize("#cfc6b8", spectators_inside_per_team))) .. "\n"
    end
  else
    teams = "---"
  end
  ...
  local p_names = ""
  for pl, _ in pairs(arena.players) do
    p_names = p_names .. " " .. pl
  end
  ...
  if mod_ref.spectate_mode then
    ...
    spectatable_entities = core.colorize("#eea160", S("Current spectatable entities: @1", core.colorize("#cfc6b8", entities))) .. "\n"
    spectatable_areas = core.colorize("#eea160", S("Current spectatable areas: @1", core.colorize("#cfc6b8", areas))) .. "\n\n"
  end
  ...
  if arena.is_resetting ~= nil then
    is_resetting = core.colorize("#eea160", S("Is resetting: @1", core.colorize("#cfc6b8", tostring(arena.is_resetting)))) .. "\n"
  end
  ...
  local region = "---"
  if arena.pos1 then
    region = "Pos1 " .. core.pos_to_string(arena.pos1) .. " Pos2 " .. core.pos_to_string(arena.pos2) .. ""
  end
  ...
  if arena.custom_return_point then
    ...
    custom_return_point = "Pos = " .. core.pos_to_string(arena.custom_return_point.pos) .. " | Rot = " .. printed_rot
  end
  ...
  if arena.teams_enabled then
    ...
    for team_ID = 1, #arena.teams do
      ...
      for _, spwn in pairs(arena.spawn_points[team_ID])  do
        spawners_pos = spawners_pos .. " " .. core.pos_to_string(spwn.pos) .. " "
      end
  ...
  if mod_ref.time_mode ~= "none" then
    ...
    time = core.colorize("#eea160", S("Initial time: @1", core.colorize("#cfc6b8", arena.initial_time .. " (" .. S("current: @1", current_time) .. ")"))) .. "\n"
  end
  ...
  if mod_ref.spectate_mode then
    ...
    sp_properties = core.colorize("#eea160", S("Spectator properties: @1", core.colorize("#cfc6b8", sp_properties))) .. "\n"
  end
  ...
  if arena.teams_enabled then
    ...
    team_properties = core.colorize("#eea160", S("Team properties: @1", core.colorize("#cfc6b8", team_properties)))
  end

  core.chat_send_player(sender,
    core.colorize("#cfc6b8", "====================================") .. "\n" ..
    core.colorize("#eea160", S("Name: @1 | (mod: @2)", core.get_color_escape_sequence("#cfc6b8") .. arena_name, arena.mod)) .. "\n" ..
    core.colorize("#eea160", S("ID: @1", core.colorize("#cfc6b8", arena_ID))) .. "\n" ..
    core.colorize("#eea160", S("Match ID: @1", core.colorize("#cfc6b8", arena.matchID or "---"))) .. "\n\n" ..

    core.colorize("#eea160", S("Author: @1", core.colorize("#cfc6b8", arena.author))) .. "\n" ..
    core.colorize("#eea160", S("Thumbnail: @1", core.colorize("#cfc6b8", thumbnail))) .. "\n" ..
    core.colorize("#eea160", S("BGM: @1", core.colorize("#cfc6b8", arena_bgm))) .. "\n\n" ..

    core.colorize("#eea160", S("Teams: @1", core.colorize("#cfc6b8", teams))) .. "\n" ..
    min_players_per_team ..
    max_players_per_team ..
    core.colorize("#eea160", S("Players required: @1", core.colorize("#cfc6b8", arena_min_players))) .. "\n" ..
    core.colorize("#eea160", S("Players supported: @1", core.colorize("#cfc6b8", arena_max_players))) .. "\n" ..
    core.colorize("#eea160", S("Players inside: @1", core.colorize("#cfc6b8", arena.players_amount .. " ( ".. p_names .. " )"))) .. "\n" ..
    players_inside_per_team ..
    core.colorize("#eea160", S("Spectators inside: @1", core.colorize("#cfc6b8", arena.spectators_amount .. " ( ".. sp_names .. " )"))) .. "\n" ..
    spectators_inside_per_team ..
    core.colorize("#eea160", S("Players and spectators inside: @1", core.colorize("#cfc6b8", psp_amount .. " ( ".. psp_names .. " )"))) .. "\n" ..
    core.colorize("#eea160", S("Past and present players: @1", core.colorize("#cfc6b8", ppp_names_amount .. " ( " .. ppp_names .. " )"))) .."\n" ..
    core.colorize("#eea160", S("Past and present players inside: @1", core.colorize("#cfc6b8", ppp_names_inside_amount .. " ( " .. ppp_names_inside .. " )"))) .."\n\n" ..

    spectatable_entities ..
    spectatable_areas ..

    core.colorize("#eea160", S("Enabled: @1", core.colorize("#cfc6b8", tostring(arena.enabled)))) .. "\n" ..
    core.colorize("#eea160", S("Status: @1", core.colorize("#cfc6b8", status))) .. "\n" ..
    is_resetting ..
    core.colorize("#eea160", S("Entrance: @1", core.colorize("#cfc6b8", "(" .. arena.entrance_type .. ") " .. entrance))) .. "\n" ..
    core.colorize("#eea160", S("Region: @1", core.colorize("#cfc6b8", region))) .. "\n" ..
    core.colorize("#eea160", S("Custom return point: @1", core.colorize("#cfc6b8", custom_return_point))) .. "\n" ..
    core.colorize("#eea160", S("Spawn points: @1", core.colorize("#cfc6b8", arena_lib.get_arena_spawners_count(arena) .. " ( " .. spawners_pos .. ")"))) .. "\n\n" ..

    time ..
    core.colorize("#eea160", S("Custom sky: @1", core.colorize("#cfc6b8", celvault))) .. "\n" ..
    core.colorize("#eea160", S("Weather: @1", core.colorize("#cfc6b8", weather))) .. "\n" ..
    core.colorize("#eea160", S("Custom lighting: @1", core.colorize("#cfc6b8", lighting))) .. "\n\n" ..

    core.colorize("#eea160", S("Properties: @1", core.colorize("#cfc6b8", properties))) .. "\n" ..
    core.colorize("#eea160", S("Temp properties: @1", core.colorize("#cfc6b8", temp_properties))) .. "\n" ..
    core.colorize("#eea160", S("Player properties: @1", core.colorize("#cfc6b8", p_properties))) .. "\n" ..
    sp_properties,
    team_properties
  )
end
...
function arena_lib.flush_arena(mod, arena_name, sender)
  ...
  core.chat_send_player(sender, "Sluuush!")
end
...
function arena_lib.check_for_unused_resources(sender)
  local resources = {}
  local wrld_path = core.get_worldpath() .. "/arena_lib"

  for _, f_name in pairs(core.get_dir_list(wrld_path .. "/BGM", false)) do
    if f_name:find(".ogg") then
      resources[f_name] = true
    end
  end

  for _, f_name in pairs(core.get_dir_list(wrld_path .. "/Thumbnails")) do
  ...
  core.chat_send_player(sender, core.colorize("#cfc6b8", "===================================="))
  core.chat_send_player(sender, core.colorize("#eea160", S("Unused arena_lib resources:")))

  if next(resources) then
    for name, _ in pairs(resources) do
      core.chat_send_player(sender, name)
    end
  else
    core.chat_send_player(sender, "---")
  end
