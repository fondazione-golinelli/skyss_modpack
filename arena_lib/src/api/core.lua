...
function arena_lib.register_minigame(mod, def)
  ...
  if def.show_minimap ~= nil then
    core.log("warning", "[ARENA_LIB] (" .. mod .. ") show_minimap is deprecated. Use hud_flags = {minimap = false/true} instead")
    if def.hud_flags then
      def.hud_flags.minimap = def.show_minimap
    else
      def.hud_flags = {minimap = def.show_minimap}
    end
  end
  ...
  if def.disabled_huds then
    core.log("warning", "[ARENA_LIB] (" .. mod .. ") disabled_huds is deprecated. Use hud_flags = {hud_name = true/false} instead")
  ...
  if def.chat_all_prefix then
    ...
    core.log("warning", "[ARENA_LIB] (" .. mod .. ") chat_all_prefix is deprecated. Use chat_settings = {prefix_all = \"some prefix\"} instead")
  end

  if def.chat_team_prefix then
    def.chat_settings.prefix_team = def.chat_team_prefix
    core.log("warning", "[ARENA_LIB] (" .. mod .. ") chat_team_prefix is deprecated. Use chat_settings = {prefix_team = \"some prefix\"} instead")
  end

  if def.chat_spectate_prefix then
    def.chat_settings.prefix_spectate = def.chat_spectate_prefix
    core.log("warning", "[ARENA_LIB] (" .. mod .. ") chat_spectate_prefix is deprecated. Use chat_settings = {prefix_spectate = \"some prefix\"} instead")
  end

  if def.chat_all_color then
    def.chat_settings.color_all = def.chat_all_color
    core.log("warning", "[ARENA_LIB] (" .. mod .. ") chat_all_color is deprecated. Use chat_settings = {color_all = \"some color\"} instead")
  end

  if def.chat_team_color then
    def.chat_settings.color_team = def.chat_team_color
    core.log("warning", "[ARENA_LIB] (" .. mod .. ") chat_team_color is deprecated. Use chat_settings = {color_team = \"some color\"} instead")
  end

  if def.chat_spectate_color then
    def.chat_settings.color_spectate = def.chat_spectate_color
    core.log("warning", "[ARENA_LIB] (" .. mod .. ") chat_spectate_color is deprecated. Use chat_settings = {color_spectate = \"some color\"} instead")
  end

  if def.spectate_mode == true then
    def.spectate_mode = "all"
    core.log("warning", "[ARENA_LIB] (" .. mod .. ") spectate_mode = true is deprecated. Use spectate_mode = \"all\" instead")
  end
  ...
  mod_ref.description = def.description or ""
  ...
  mod_ref.chat_settings = {
    ...
    isolate_chat = true,
    can_interact_with_spectators = false
  }
  mod_ref.messages = {
  ...
  mod_ref.in_game_physics = table.copy(physics_default)
  ...
  if def.chat_settings and type(def.chat_settings) == "table" then
    ...
    if new_settings.isolate_chat == false then
      settings.isolate_chat = false
    end

    if new_settings.can_interact_with_spectators == true then
      settings.can_interact_with_spectators = true
    end
  end
  ...
  if def.sounds then
    ...
    for k, audio in pairs(def.sounds) do
      ...
      if type(audio) == "table" then
        ...
        audio_lib.register_sound("notifications", audio_name, audio.description, audio.params)
        mod_ref.sounds[k] = audio_name
      else  -- o `false`
  ...
  -- TODO in futuro hp_max e fov in tabella player_attributes
  if def.hp_max then
    mod_ref.hp_max = def.hp_max
  end
  ...
  if def.camera_mode then
    mod_ref.camera_mode = def.camera_mode
  end
  ...
  if def.keep_physics == true then
    mod_ref.keep_physics = true
  end
  ...
  if def.time_mode then
    ...
    core.after(0.1, function()  -- deve caricare la registrazione del richiamo
  ...
  if def.in_game_physics and type(def.in_game_physics) == "table" then
    for k, v in pairs(def.in_game_physics) do
      mod_ref.in_game_physics[k] = v
    end
  end
...
function arena_lib.register_entrance_type(mod, entrance, def)
  ...
  core.register_tool( mod ..":editor_entrance", {
...
function arena_lib.change_mod_settings(sender, mod, setting, new_value)
  ...
  storage:set_string(mod .. ".SETTINGS", core.serialize(mod_settings))
...
function arena_lib.set_thumbnail(sender, mod, arena_name, thumbnail, in_editor)
  ...
  elseif thumbnail == nil or thumbnail == "" then
    ...
    local thmb_dir = core.get_worldpath() .. "/arena_lib/Thumbnails/"
...
function arena_lib.set_region(sender, mod, arena_name, pos1, pos2, in_editor)
  ...
  if not pos1 and not pos2 then
    ...
    arena.pos1 = vector.round(p1_sort)
    arena.pos2 = vector.round(p2_sort)
  end

  local warn_on_sorting = arena.pos1 and S("(pos1 and pos2 have been automatically sorted)") or ""

  arena_lib.update_waypoints(sender, mod, arena)
  delete_schematic_if_exists(sender, mod, id)
  update_storage(false, mod, id, arena)
  arena_lib.print_info(sender, arena_lib.mods[mod].prefix .. S("Region of arena @1 successfully overwritten", arena_name) .. " " .. warn_on_sorting)
end
...
function arena_lib.enable_arena(sender, mod, arena_name, in_editor)
  ...
  if arena.pos1 then
    local v1, v2  = arena.pos1, arena.pos2
  ...
  if arena_lib.is_player_in_edit_mode(sender) then
    arena_lib.quit_editor(core.get_player_by_name(sender))
  end
...
function arena_lib.store_inventory(player)
  ...
  storage:set_string(player:get_player_name() .. ".INVENTORY", core.serialize(stored_inv))
...
function arena_lib.restore_inventory(p_name)
  ...
  if storage:get_string(p_name .. ".INVENTORY") ~= "" then
    ...
    local stored_inv = core.deserialize(storage:get_string(p_name .. ".INVENTORY"))
    local current_inv = core.get_player_by_name(p_name):get_inventory()
...
function arena_lib.load_maps_from_db()
  local map = core.deserialize(storage:get_string("maps"))
...
function arena_lib.save_maps_onto_db(table)
  storage:set_string("maps", core.serialize(table))
end
-- ^ ------------------------------------ ^
...
function load_settings(mod)
  ...
  if storage:get_string(mod .. ".SETTINGS") == "" then
    ...
    storage:set_string(mod .. ".SETTINGS", core.serialize(default_settings))
  else
    arena_lib.mods[mod].settings = core.deserialize(storage:get_string(mod .. ".SETTINGS"))
    ...
    if not arena_lib.mods[mod].settings.return_point.pos then
      ...
      storage:set_string(mod .. ".SETTINGS", core.serialize(arena_lib.mods[mod].settings))
    end
  end
end
...
function init_storage(mod, mod_ref)
  ...
  for i = 1, arena_lib.mods[mod].highest_arena_ID do
    ...
    if arena_str ~= "" then
      local arena = core.deserialize(arena_str)
      ...
      if next(arena.spawn_points) then
        ...
        if update_spawners then
          ...
          core.log("action", "[ARENA_LIB] spawn points of arena " .. arena.name ..
      ...
      if arena.pos1 then
        ...
        if not vector.equals(p1, arena.pos1) then
          arena.pos1 = p1
          arena.pos2 = p2
          to_update = true
        end
      ...
      elseif arena.teams_enabled and not mod_ref.variable_teams_amount and #mod_ref.teams ~= #arena.teams then
        ...
        arena.enabled = false
        to_update = true
        core.log("action", "[ARENA_LIB] teams amount of arena " .. arena.name .. " has changed: resetting arena spawn points")
  ...
  core.log("action", "[ARENA_LIB] Mini-game " .. mod .. " loaded")
end
...
function update_storage(erase, mod, id, arena)
  ...
  if erase then
    ...
    storage:set_string(entry, core.serialize(arena))
  end
end
...
function check_for_properties(mod, mod_ref)
  ...
  elseif not has_old_properties and has_new_properties then
    core.log("action", "[ARENA_LIB] Properties have been declared. Proceeding to add them")

  -- se c'erano prima e ora non ci sono più, svuoto e annullo
  elseif has_old_properties and not has_new_properties then

    for property, _ in pairs(core.deserialize(old_properties)) do
      for id, arena in pairs(mod_ref.arenas) do
        arena[property] = nil
        update_storage(false, mod, id, arena)
      end
    ...
    core.log("action", "[ARENA_LIB] There are no properties left in the declaration of the mini-game. They've been removed from arenas")
  ...
  -- se c'erano sia prima che ora, le confronto
    ...
    local old_properties_table = core.deserialize(old_properties) -- chiave: num,       valore: proprietà
    ...
    if #old_properties_table == #new_properties_table and not next(old_swapped) then
      ...
      core.log("action", "[ARENA_LIB] Properties have changed. Proceeding to modify old arenas")
    end
  end

  local old_table = core.deserialize(old_properties)
  ...
  for property, v in pairs(mod_ref.properties) do
    if old_properties_table[property] == nil then
      ...
      core.log("action", "[ARENA_LIB] Adding property " .. property)
  ...
  for old_property, _ in pairs(old_properties_table) do
    if mod_ref.properties[old_property] == nil then
      core.log("action", "[ARENA_LIB] Removing property " .. old_property)
  ...
  storage:set_string(mod .. ".PROPERTIES", core.serialize(new_properties_table))
end
...
function arena_lib.force_arena_ending(mod, arena, sender)
  core.log("warning", "[ARENA_LIB] (" .. mod .. ") force_arena_ending is deprecated. Please use force_end instead")
  arena_lib.force_end(sender, mod, arena)
end
...
function arena_lib.get_player_spectated(sp_name)
  ...
  core.log("warning", "[ARENA_LIB] (" .. mod .. ") get_player_spectated is deprecated. Please use get_spectated_target instead")
...
function arena_lib.get_spectate_entities(mod, arena_name)
  core.log("warning", "[ARENA_LIB] (" .. mod .. ") get_spectate_entities is deprecated. Please use get_spectatable_entities instead")
  return arena_lib.get_spectatable_entities(mod, arena_name)
end

function arena_lib.get_spectate_areas(mod, arena_name)
  core.log("warning", "[ARENA_LIB] (" .. mod .. ") get_spectate_areas is deprecated. Please use get_spectatable_areas instead")
