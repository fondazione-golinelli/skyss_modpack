...
function arena_lib.join_queue(mod, arena, p_name)
  ...
  if arena_lib.is_player_in_queue(p_name) then
    ...
    -- se era in coda per la stessa arena, interrompo qua, sennò procedo per aggiungerlə nella nuova
  ...
  if queue_joins[p_name] and queue_joins[p_name] >= 3 then
    ...
    if queue_joins[p_name] == 3 then  -- to avoid using a 2nd bool parameter to run the after just once
      ...
      core.after(10, function() queue_joins[p_name] = nil end)
    end
    return
  end
  ...
  for _, pl_name in ipairs(players_to_add) do
    arena_lib.send_message_in_arena(arena, "both", core.colorize("#c8d692", arena_name .. " > " ..  pl_name))
  ...
  if not arena.in_queue and not arena.in_game then
    ...
    -- ...e ci sono abbastanza giocanti, parte il timer d'attesa
  ...
  for _, pl_name in ipairs(players_to_add) do
    if mod_ref.on_join_queue then
      mod_ref.on_join_queue(arena, pl_name)
    end

    for _, callback in ipairs(arena_lib.registered_on_join_queue) do
      callback(mod, arena, pl_name, has_queue_status_changed)
    end
  end
...
function arena_lib.remove_player_from_queue(p_name, has_disconnected)
  ...
  if is_player_in_party(p_name) then
    -- (se non è lə capogruppo e non si è disconnessə, annullo)
  ...
  for pl_name, _ in pairs(players_to_remove) do
    ...
    if arena_lib.is_player_in_queue(pl_name) then
      ...
      arena_lib.send_message_in_arena(arena, "both", core.colorize("#d69298", arena_name .. " < " .. pl_name))
      ...
      if arena.teams_enabled then
        ...
        -- non controllo con is_player_in_party(..) perché potrebbe anche essere entratə
        -- in coda mentre era in gruppo, per poi uscire dal gruppo (rimanendo in coda).
        -- Quindi sempre meglio provare a svuotare
        if p_team_ID then
  ...
  for pl_name, _ in pairs(players_to_remove) do
    if mod_ref.on_leave_queue then
      mod_ref.on_leave_queue(arena, pl_name)
    end
...
function arena_lib.queue_to_arena(mod, arena)
  ...
  for pl_name, _ in pairs(arena.players) do
    ...
    -- rimuovo eventuali giocanti in gruppo che stanno seguendo altre partite.
    -- remove_player_from_arena ottiene l'arena tramite get_arena_by_player, che
    -- come primo controllo verifica se lə giocante sta in partita. Ergo prende l'arena
    -- che sta venendo seguita (al momento lə giocante si trova in due arene
    -- contemporaneamente), rimuovendolə da quest'ultima (e non da questa nuova)
    if is_player_in_party(pl_name) and arena_lib.is_player_spectating(pl_name) then
  ...
  if arena.teams_enabled then
    ...
    for pl_name, pl_data in pairs(arena.players) do
      ...
      if not pl_data.teamID or not arena.teams[pl_data.teamID] then
        core.log("warning", "[ARENA_LIB] Teams assignment error! Arena = " .. arena.name .. ", player = " .. pl_name .. ", players = " .. dump(arena.players))
      end

      core.chat_send_player(pl_name, mod_ref.prefix .. S("You've joined team @1", core.colorize("#eea160", arena.teams[pl_data.teamID].name)))
    end
  end
...
function is_player_in_party(p_name)
  return core.get_modpath("parties") and parties.is_player_in_party(p_name)
end
