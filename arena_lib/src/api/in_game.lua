...
local function remove_and_store_attachments() end
local function delete_ingame_attachments() end
...
local function region_check_loop() end
...
local players_temp_storage = {}       -- KEY: p_name, VALUE:    {(int) hotbar_slots, (string) hotbar_background_image, (string) hotbar_selected_image,
                                      --                        (table) player_aspect, (int) fov, (string) camera_mode, (table) camera_offset, (table) armor_groups,
                                      --                        (string) inventory_fs, (table) attachments, (table) nametag, (table) celvault_sky,
                                      --                        (table) celvault_sun, (table) celvault_moon, (table) celvault_stars, (table) celvault_clouds, (int) weather_ID}
...
function arena_lib.load_arena(mod, arena_ID)
  ...
  for pl_name, pl_data in pairs(arena.players) do
    ...
    if not pl_name:find(" ") then -- TEMP: escludi i fantocci. Serve https://github.com/luanti-org/luanti/issues/13430
  ...
  core.after(mod_ref.load_time, function()
...
function arena_lib.start_arena(mod, arena)
  ...
  if not arena.in_loading then
    core.log("error", debug.traceback("[" .. arena.name .. "] There has been an attempt to call the fighting phase whilst already in it. This shall not be done, aborting..."))
    return end
  ...
  if arena.pos1 then
    region_check_loop(mod_ref, arena)
  end
...
function arena_lib.join_arena(mod, p_name, arena_ID, as_spectator)
  ...
  if as_spectator then
    ...
    -- controllo che non sia fantoccio
    if core.get_player_by_name(p_name) then
      -- se si è attaccatɜ a qualcosa
      if core.get_player_by_name(p_name):get_attach() then
    ...
    on_join_callbacks(mod_ref, mod, arena, {[p_name] = {}}, as_spectator)
    arena_lib.enter_spectate_mode(p_name, arena)
    arena_lib.send_message_in_arena(arena, "both", core.colorize("#cfc6b8", ">>> " .. p_name .. " (" .. S("spectator") .. ")"))
  ...
  -- se entra come giocante
    ...
    local joining_players = {p_name}

    if IS_PARTIES_ENABLED and parties.is_player_in_party(p_name) then
      joining_players = parties.get_party_members(p_name, true)
    end

    -- Passaggi per l'inserimento in partita:
    -- (1) gestione spettante -> (2) inizializzazione in arena -> (3) aggiunta in partita

    for _, pl_name in ipairs(joining_players) do
      players_join_data[pl_name] = {}

      -- (1) prima rimuovi eventuali persone che seguivano partite al di fuori dell'arena
      -- selezionata. Se eseguito dopo (2), per uno step lə giocante viene consideratə
      -- sia spettante in arena A che giocante in arena B (disastro annunciato)
      if arena_lib.is_player_spectating(pl_name) then
        ...
        if pl_arena.name == arena.name and pl_mg == mod then
          ...
          -- Teletrasporto in un punto qualsiasi dell'arena nuova, per evitare che
          -- i controlli di on_region_trespass considerino per uno step lə giocante
          -- fuori dall'area (di base uccidendolə)
          core.get_player_by_name(pl_name):set_pos(arena_lib.get_random_spawner(arena, nil, true).pos)
        end
      end
    end

    -- (2) inizializza in arena
    arena_lib.get_and_add_joining_players(arena, p_name)

    -- (3) inserisci in partita
    for _, pl_name in ipairs(joining_players) do
      if players_join_data[pl_name].was_spectator then
        arena_lib.leave_spectate_mode(pl_name) -- va chiamato qui e non in (1) perché sennò (2) crede che non essendo spettante sia giocante in partita e non lə inizializza
        core.get_player_by_name(pl_name):get_inventory():set_list("main", {}) -- rimuovo oggetti della spettatore
      ...
      arena_lib.send_message_in_arena(arena, "both", core.colorize("#c6f154", " >>> " .. pl_name))

      local teamID = arena.players[pl_name].teamID
      local random_spawner = arena_lib.get_random_spawner(arena, teamID, true)
      local player = core.get_player_by_name(pl_name)

      -- TEMP: escludi i fantocci. Serve https://github.com/luanti-org/luanti/issues/13430
      if player then
...
function arena_lib.load_celebration(mod, arena, winners)
  ...
  if arena.in_celebration then
    core.log("error", debug.traceback("[" .. mod .. "] There has been an attempt to call the celebration phase whilst already in it. This shall not be done, aborting..."))
    return end

  local mod_ref = arena_lib.mods[mod]

  if mod_ref.endless then
    core.log("error", debug.traceback("[" .. mod .. "] There has been an attempt to call the celebration phase for an endless minigame, aborting..."))
    return end
  ...
  if winners_type == "table" and #winners > 1 then
    if type(winners[1]) == "string" then
      ...
      local mod_S = mod_ref.custom_messages.celebration_more_players and core.get_translator(mod) or S
      ...
      local mod_S = mod_ref.custom_messages.celebration_more_teams and core.get_translator(mod) or S
  ...
  elseif winners == nil or (winners_type == "table" and not next(winners)) then
    local mod_S = mod_ref.custom_messages.celebration_nobody and core.get_translator(mod) or S
  ...
  elseif winners_type == "string" or (winners_type == "table" and type(winners[1]) == "string") then
    local mod_S = mod_ref.custom_messages.celebration_one_player and core.get_translator(mod) or S
  ...
  elseif winners_type == "number" or (winners_type == "table" and type(winners[1]) == "number") then
    local mod_S = mod_ref.custom_messages.celebration_one_team and core.get_translator(mod) or S
  ...
  if mod_ref.spectate_mode and not mod_ref.chat_settings.can_interact_with_spectators then
    arena_lib.send_message_in_arena(arena, "both", core.colorize("#cfc6b8", "> " .. S("Players and spectators can now interact with each other")))
  end
  ...
  core.after(mod_ref.celebration_time, function()
...
function arena_lib.remove_player_from_arena(p_name, reason, xc_name, elim_msg)
  ...
  local p_in_game_data = {} -- dati aggiuntivi

  -- potrebbe essere fantoccio
  if player then
    p_in_game_data = {
      last_pos = core.get_player_by_name(p_name):get_pos()
    }
  end
  ...
  if mod_ref.spectate_mode and arena_lib.is_player_spectating(p_name) then
    ...
    handle_leaving_callbacks(mod, arena, p_name, p_properties, p_in_game_data, reason, xc_name, elim_msg, true)
  ...
  -- sennò...
    ...
    if reason == 1 and mod_ref.spectate_mode and arena.players_amount > 0 then
      eliminate_player(mod, arena, p_name, xc_name, p_properties, p_in_game_data, elim_msg)
    ...
    -- sennò procedo a rimuoverlo normalmente
      ...
      handle_leaving_callbacks(mod, arena, p_name, p_properties, p_in_game_data, reason, xc_name, elim_msg)
    end
    ...
    if not mod_ref.endless and not arena.in_celebration then
      ...
      elseif mod_ref.end_when_too_few then
        ...
        if arena.teams_enabled and #arena_lib.get_active_teams(arena) == 1 then
          ...
          local mod_S = mod_ref.custom_messages.last_standing_team and core.get_translator(mod) or S
        ...
        elseif arena.players_amount == 1 then
          local mod_S = mod_ref.custom_messages.last_standing and core.get_translator(mod) or S
...
function arena_lib.force_end(sender, mod, arena)
  ...
  arena_lib.send_message_in_arena(arena, "both", core.colorize("#d69298", S("The arena has been forcibly terminated by @1", sender)))
  ...
  if to_player then
    for pl_name, info in pairs(players_in_game) do
      if mod == info.minigame then
        table.insert(players_in_minigame, core.get_player_by_name(pl_name))
      end
    end
  else
    for pl_name, info in pairs(players_in_game) do
...
function operations_before_entering_arena(mod_ref, mod, arena, arena_ID, p_name, as_spectator)
  ...
  -- TEMP: escludi i fantocci. Serve https://github.com/luanti-org/luanti/issues/13430
  if p_name:find(" ") then players_in_game[p_name] = {minigame = mod, arenaID = arena_ID} return end

  -- applico eventuale musica di sottofondo
  if arena.bgm then
    local bgm_table = arena.bgm
    audio_lib.play_bgm(p_name, bgm_table.track, {gain = bgm_table.gain, pitch = bgm_table.pitch})
  end

  local player = core.get_player_by_name(p_name)
  local p_storage = players_temp_storage[p_name]
  ...
  if arena.celestial_vault then
    ...
    if celvault.sky then
      p_storage.celvault_sky = player:get_sky(true)
      player:set_sky(celvault.sky)
    end

    if celvault.sun then
      p_storage.celvault_sun = player:get_sun()
      player:set_sun(celvault.sun)
    end

    if celvault.moon then
      p_storage.celvault_moon = player:get_moon()
      player:set_moon(celvault.moon)
    end

    if celvault.stars then
      p_storage.celvault_stars = player:get_stars()
      player:set_stars(celvault.stars)
    end

    if celvault.clouds then
      p_storage.celvault_clouds = player:get_clouds()
  ...
  if arena.weather then
    ...
    p_storage.weather_ID = core.add_particlespawner(particles)
  end

  -- salvo la targhetta ed eventualmente la nascondo
  p_storage.nametag = player:get_nametag_attributes()
  ...
  if next(mod_ref.hud_flags) then
    p_storage.hud_flags = player:hud_get_flags()
    player:hud_set_flags(mod_ref.hud_flags)
  end

  -- chiudo eventuali formspec
  core.close_formspec(p_name, "")
  ...
  if mod_ref.spectate_mode or mod_ref.hotbar then
    p_storage.hotbar_slots = player:hud_get_hotbar_itemcount()
    p_storage.hotbar_background_image = player:hud_get_hotbar_image()
    p_storage.hotbar_selected_image = player:hud_get_hotbar_selected_image()
  end
  ...
  if not mod_ref.keep_attachments and next(player:get_children()) then
    p_storage.attachments = {}
    remove_and_store_attachments(p_name, player)
  end

  -- eventuale vita massima personalizzata (anche su spettanti dato che vedono la
  -- vita di chi seguono)
  if mod_ref.hp_max then
    p_storage.hp_max = player:get_properties().hp_max
    player:set_properties({hp_max = mod_ref.hp_max})
  end
...
function operations_before_playing_arena(mod_ref, arena, p_name)
  ...
  if mod_ref.fov then
    p_storage.fov = player:get_fov()
    player:set_fov(mod_ref.fov)
  end

  -- applico eventuale modalità camera
  if mod_ref.camera_mode then
    p_storage.camera_mode = player:get_camera()
    player:set_camera({mode = mod_ref.camera_mode})
  end

  -- applico eventuale scostamento camera
  if mod_ref.camera_offset then
    p_storage.camera_offset = {player:get_eye_offset()}
  ...
  if arena.teams_enabled and mod_ref.teams_color_overlay then
    if not p_storage.player_aspect then -- salvo texture in player_aspect per non creare un ulteriore parametro
      p_storage.player_aspect = {}
    end
    ...
    p_storage.player_aspect.textures = player:get_properties().textures
    player:set_properties({ textures = textures })
  end

  -- disabilito eventualmente l'inventario
  if mod_ref.disable_inventory then
    p_storage.inventory_fs = player:get_inventory_formspec()
  ...
  if not mod_ref.keep_physics then
  ...
  if next(mod_ref.damage_modifiers) then
    p_storage.armor_groups = player:get_armor_groups()
  ...
  if table.indexof(mod_ref.disabled_damage_types, "fall") > 0 then
    ...
    if not p_storage.armor_groups then
      p_storage.armor_groups = player:get_armor_groups()
    end
...
-- `reason` parametro opzionale che passo solo quando potrebbe essersi disconnessə
function operations_before_leaving_arena(mod_ref, arena, p_name, reason)
  -- svuoto prima players_in_game, così is_in_arena torna false su :respawn() più
  -- in basso; altrimenti, dato che in player_manager.lua controllo se è in arena
  -- quando resuscitano (usando is_in_arena), basta qualche on_respawn di un minigioco
  -- che usa x es. arena.players[p_name] x far crashare (dato che [p_name] già non
  -- esiste più)
  players_in_game[p_name] = nil

  if p_name:find(" ") then return end -- TEMP: escludi i fantocci. Serve https://github.com/luanti-org/luanti/issues/13430
  ...
  if arena.lighting then
    player:override_day_night_ratio(p_storage.lighting.light)
    player:set_lighting(p_storage.lighting.shaders)
  end
  ...
  if arena.celestial_vault then
    ...
    if celvault.sky then
      player:set_sky(p_storage.celvault_sky)
    end
    if celvault.sun then
      player:set_sun(p_storage.celvault_sun)
    end
    if celvault.moon then
      player:set_moon(p_storage.celvault_moon)
    end
    if celvault.stars then
      player:set_stars(p_storage.celvault_stars)
    end
    if celvault.clouds then
      player:set_clouds(p_storage.celvault_clouds)
    end
  end

  -- rimuovo eventuale effetto meteo
  if arena.weather then
    core.delete_particlespawner(p_storage.weather_ID)
  end
  ...
  local armor_groups = p_storage.armor_groups
  ...
  if not mod_ref.keep_attachments then
    delete_ingame_attachments(player)

    local attachments = p_storage.attachments or {}
    for i, _ in pairs(attachments) do
      restore_attachments(p_name, player, i)
    end
  end

  -- ripristino eventuali hp massimi
  if mod_ref.hp_max then
    player:set_properties({hp_max = p_storage.hp_max})
  end
  ...
  -- TEMP: waiting for https://github.com/minetest/minetest/issues/12092. Forcing the teleport twice on two different steps
  -- Non eseguire se sta in un'altra arena, ovvero se da spettante arena A è passatə
  -- a giocante arena B (perché in gruppo e non capəgruppo)
  core.after(0.1, function()
    if not core.get_player_by_name(p_name) or arena_lib.is_player_in_arena(p_name) then return end
    player:set_pos(noise_pos)
  end)
  ...
  -- se si è disconnessə, salta il resto
  if reason == 0 then
  ...
  if arena.past_present_players_inside[p_name] then
    ...
    if arena.teams_enabled and mod_ref.teams_color_overlay then
      player:set_properties({
        textures = p_storage.player_aspect.textures
      })
    end

    -- riprsitino eventuale aspetto
    if mod_ref.player_aspect then
      local aspect = p_storage.player_aspect
      player:set_properties({
        visual = aspect.visual, mesh = aspect.mesh, textures = aspect.textures, visual_size = aspect.visual_size, collisionbox = aspect.collisionbox, selectionbox = aspect.selectionbox
      })
    end
    ...
    if mod_ref.fov then
      player:set_fov(p_storage.fov)
    end

    -- ripristino eventuale modalità camera
    if mod_ref.camera_mode then
      local prev_params = p_storage.camera_mode

      -- se `"any"`, forzo la prima persona prima, in quanto è la più usata
      if prev_params.mode == "any" then
        ...
        player:set_camera(prev_params)
      end
    end

    -- ripristino eventuale scostamento camera
    if mod_ref.camera_offset then
  ...
  if mod_ref.spectate_mode or mod_ref.hotbar then
    player:hud_set_hotbar_itemcount(p_storage.hotbar_slots)
    player:hud_set_hotbar_image(p_storage.hotbar_background_image)
    player:hud_set_hotbar_selected_image(p_storage.hotbar_selected_image)
  end

  -- reimposto la fisica
  if core.get_modpath("hub") then
  ...
  if next(mod_ref.hud_flags) then
    player:hud_set_flags(p_storage.hud_flags)
  end

  -- ripristino nomi
  player:set_nametag_attributes(p_storage.nametag)
...
function remove_and_store_attachments(p_name, entity, parent_idx)
  for i, child in pairs(entity:get_children()) do
    ...
    if luaentity then
      ...
      if luaentity.initial_properties.static_save then
        ...
        for _, grandchild in pairs(child:get_children()) do
        ...
        remove_and_store_attachments(p_name, child, i)
      end
...
function delete_ingame_attachments(obj)
  for _, child in ipairs(obj:get_children()) do
    for _, grandchild in ipairs(child:get_children()) do
      grandchild:remove()
    end
    child:remove()
  end
...
function restore_attachments(p_name, parent, i)
  ...
  local child = core.add_entity(parent:get_pos(), data.entity.name, data.entity.staticdata)
...
function eliminate_player(mod, arena, p_name, xc_name, p_properties, p_in_game_data, elim_msg)
  local mod_ref = arena_lib.mods[mod]

  if elim_msg then
    arena_lib.send_message_in_arena(arena, "both", core.colorize("#f16a54", "<<< " .. elim_msg))
  else
    if xc_name then
      local mod_S = mod_ref.custom_messages.eliminated_by and core.get_translator(mod) or S
      arena_lib.send_message_in_arena(arena, "both", core.colorize("#f16a54", "<<< " .. mod_S(mod_ref.messages.eliminated_by, p_name, xc_name)))
    else
      local mod_S = mod_ref.custom_messages.eliminated and core.get_translator(mod) or S
      arena_lib.send_message_in_arena(arena, "both", core.colorize("#f16a54", "<<< " .. mod_S(mod_ref.messages.eliminated, p_name)))
    end
  end

  if mod_ref.sounds.eliminate then
    for psp_name, _ in pairs(arena.players_and_spectators) do
      audio_lib.play_sound(mod_ref.sounds.eliminate, {to_player = psp_name})
    end
  end

  if mod_ref.on_eliminate then
    mod_ref.on_eliminate(arena, p_name, xc_name, p_properties, p_in_game_data)
  end

  for _, callback in ipairs(arena_lib.registered_on_eliminate) do
    callback(mod, arena, p_name, xc_name, p_properties, p_in_game_data)
  end
end



function handle_leaving_callbacks(mod, arena, p_name, p_properties, p_in_game_data, reason, xc_name, elim_msg, is_spectator)
  ...
  -- se si è disconnesso
  if reason == 0 then
    arena_lib.send_message_in_arena(arena, "both", core.colorize(msg_color, "<<< " .. p_name .. spect_str))
  ...
  elseif reason == 2 then
    if xc_name then
      arena_lib.send_message_in_arena(arena, "both", core.colorize(msg_color, "<<< " .. S("@1 has been kicked by @2", p_name, xc_name) .. spect_str))
    else
      arena_lib.send_message_in_arena(arena, "both", core.colorize(msg_color, "<<< " .. S("@1 has been kicked", p_name) .. spect_str))
    end
    ...
    core.after(120, function() players_kicked[p_name] = nil end)

  -- se ha abbandonato
  elseif reason == 3 then
    local mod_S = mod_ref.custom_messages.quit and core.get_translator(mod) or S
    arena_lib.send_message_in_arena(arena, "both", core.colorize(msg_color, "<<< " .. mod_S(mod_ref.messages.quit, p_name) .. spect_str))
  ...
  if mod_ref.on_quit then
    mod_ref.on_quit(arena, p_name, is_spectator, reason, p_properties, p_in_game_data)
  end

  for _, callback in ipairs(arena_lib.registered_on_quit) do
    callback(mod, arena, p_name, is_spectator, reason, p_properties, p_in_game_data)
  end
end
...
function victory_particles(arena, players, winners)
  ...
  if type(winners) == "string" then
    local winner = core.get_player_by_name(winners)
  ...
  elseif type(winners) == "number" then
    for pl_name, pl_stats in pairs(players) do
      if pl_stats.teamID == winners then
        local winner = core.get_player_by_name(pl_name)
...
function region_check_loop(mod_ref, arena)
  if arena.in_celebration or not arena.in_game then return end
  ...
  for pl_name, _ in pairs(arena.players) do
    ...
    if not vector.in_area(p_pos, arena.pos1, arena.pos2) then
      ps_outside[#ps_outside+1] = player
    end
  end

  if mod_ref.on_region_trespass then
    mod_ref.on_region_trespass(arena, ps_outside)
  else
    for _, pl in ipairs(ps_outside) do
      arena_lib.print_error(pl:get_player_name(), S("You can't leave the game area!"))
      pl:set_hp(0)
    end
  end
