...
function audio_lib.open_settings(p_name)
  ...
  core.show_formspec(p_name, "audio_lib:p_settings", get_formspec(p_name))
end
...
core.register_on_player_receive_fields(function(player, formname, fields)
  ...
  if fields.close or fields.quit then
    ...
    core.close_formspec(p_name, "audio_lib:p_settings")
  ...
  elseif fields.apply then
    local settings = {
      volume_main = core.explode_scrollbar_event(fields.volume_main).value,
      volume_bgm = core.explode_scrollbar_event(fields.volume_bgm).value,
      volume_sfx = core.explode_scrollbar_event(fields.volume_sfx).value,
      accessibility = temp_settings[p_name].accessibility
    }

    for _, s_type in pairs(audio_lib.get_types()) do
      local vol_setting = "volume_" .. s_type
      settings[vol_setting] = core.explode_scrollbar_event(fields[vol_setting]).value
    end
